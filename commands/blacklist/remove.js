...
module.exports = (entry) => {
  ...
  if (config.blacklist.includes(entry)) {
    config.blacklist = config.blacklist.filter((item) => item !== entry);
    fs.writeFileSync(configPath, "module.exports = "+JSON.stringify(config, null, 2), 'utf-8');
    console.log(chalk.green('◆'), chalk.reset("removed"),chalk.yellow(entry),chalk.reset("from blacklist"));
  } else {
    console.log(chalk.red('◆'), chalk.red('entry not found'));
  }
};
